...
9. `app/api/account/delete-self/route.ts` — Self-service account deletion. Caller sends their Firebase ID token as a Bearer header; route removes their `allowedUsers/{email}` Firestore doc and deletes their Firebase Auth account.
10. `app/api/admin/remove-portal-user/route.ts` — Admin-only user removal. Verifies caller is `role: 'admin'` in Firestore before removing the target from `allowedUsers` and deleting their Firebase Auth account.
